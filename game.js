...
document.addEventListener("DOMContentLoaded", function () {
    ...
    engine.spriteImages = spriteImages;
    preloadImages();

    engine.sprites.push(new Spaceship(100, 100, engine.spriteImages["ship0"], [player1_binds["thrust"], player1_binds["left"], player1_binds["right"], player1_binds["fire"], player1_binds["warp"]], 6, 0, engine));
...
function preloadImages() {
    for (const key in engine.spriteImages) {
        for (let i = 0; i < 16; i++) {
            const img = new Image();
            img.src = engine.spriteImages[key] + String(i).padStart(3, "0") + ".png";
        }
    }
}
